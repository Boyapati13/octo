...
def test_change_linux_brightness_xrandr(mock_check_output, mock_run):
    ...
    cs._change_linux_brightness_xrandr(0.1)

    # Assert check_output was called twice correctly
    assert mock_check_output.call_count == 2
    mock_check_output.assert_any_call(["xrandr"], text=True)
    mock_check_output.assert_any_call(["xrandr", "--verbose"], text=True)
...
def test_change_linux_brightness_xrandr_bounds(mock_check_output, mock_run):
    ...
    cs._change_linux_brightness_xrandr(-0.2)
    mock_run.assert_called_once()
...
@patch('actions.computer_settings.subprocess.run')
@patch('actions.computer_settings._change_linux_brightness_xrandr')
