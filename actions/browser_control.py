...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".OCTO_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".OCTO_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using OCTO profile")
                OCTO = str(Path.home() / ".OCTO_profiles" / "firefox_OCTO")
                Path(OCTO).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(OCTO, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".OCTO_profiles" / "safari")
        ...
        OCTO_profile = str(Path.home() / ".OCTO_profiles" / self.browser_name)
        Path(OCTO_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with OCTO profile: {OCTO_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(OCTO_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            print(f"[Browser] ✅ Launched [{label}] with OCTO profile")
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "OCTO_screenshot.png")
