...
_SYSTEM_PROMPT = (
    "You are OCTO, an advanced AI assistant. "
...
def screen_process(
    ...
    try:
        ...
        msg = f"Could not capture {angle}: {e}"
        print(f"[Vision] {msg}")
        if player:
            player.write_log(f"OCTO: {msg}")
        if speak:
            speak(msg)
        return False
    ...
    print(f"[Vision] Response: {answer[:120]}")
    if player:
        player.write_log(f"OCTO: {answer}")
    if speak:
        speak(answer)
    return True