...
PLANNER_PROMPT = """You are the planning module of OCTO, a personal AI assistant.
...
def create_plan(goal: str, context: str = "") -> dict:
    ...
    try:
        text = text_llm.ask(user_input, system=PLANNER_PROMPT).strip()
...
Create a REVISED plan for the remaining work only. Do not repeat completed steps."""
    ...
    try:
        text = text_llm.ask(prompt, system=PLANNER_PROMPT).strip()
        text     = re.sub(r"```(?:json)?", "", text).strip().rstrip("`").strip()
        plan     = json.loads(text)
